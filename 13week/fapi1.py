...
def login_form(userid: str = Form(...), userpassword: str = Form(...)):
    ...
    if result:
        # JSONResponse로 명시적으로 UTF-8 설정
        return JSONResponse(content={"msg": f"{userid}님 반갑습니다."}, media_type="application/json; charset=utf-8")
    else:
        return JSONResponse(content={"msg": "로그인에 실패했습니다."}, media_type="application/json; charset=utf-8")
